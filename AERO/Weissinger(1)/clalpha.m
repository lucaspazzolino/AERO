...
[gamma_cessna, cl_alpha_cessna] = weissenger_method_cl(b_cessna, c_cessna, S_cessna, S_tail_cessna, alpha);

% Calcolare la distribuzione della portanza per il Piper Cub
[gamma_piper, cl_alpha_piper] = weissenger_method_cl(b_piper, c_piper, S_piper, S_tail_piper, alpha);
...
figure;
subplot(3,1,1); % Aggiungi un terzo subplot
...
subplot(3,1,2); % Aggiungi un secondo subplot
...
% Traccia la curva C_L/\alpha
subplot(3,1,3); % Terzo subplot
plot(alpha_range, cl_alpha_cessna*alpha, 'b', alpha_range, cl_alpha_piper*alpha, 'r');
xlabel('Angolo di attacco (°)');
ylabel('C_L');
legend('Cessna 172', 'Piper Cub');
title('Curva C_L/\alpha');
